...
module.exports = {
	...
	downloadInstagram: async function (url, api, event, path) {
		try {
			...
			if (fs.statSync(path).size / 10299999 / 10299999 > 2999999) {
				return api.sendMessage("The file is too large, cannot be sent", event.threadID, () => fs.unlinkSync(path), event.messageID);
			}

			const shortUrl = await shortenURL(res);
			const messageBody = `✅🔗  Download successful!\nUrl: ${shortUrl}\n\nDont forget to add my owner:https://www.facebook.com/profile.php?id=100052395031835`;
	...
	getLink: function (url, api, event, path) {
		return new Promise((resolve, reject) => {
			if (url.includes("instagram")) {
				...
				reject(new Error("Unsupported platform. Only Instagram, Facebook,YouTube and TikTok are supported."));
			}
		});
	},
